...
public class Storage {
    ...
    public File getSdCardDirectory() {
        //return  mContext.getExternalFilesDirs(Environment.DIRECTORY_DOWNLOADS)[getSdCardDirectoryIndex()];
        return  EnvironmentEx.getExternalStoragePath();
    }

    /*public int getSdCardDirectoryIndex(){
        File[] paths = mContext.getExternalFilesDirs(Environment.DIRECTORY_DOWNLOADS);
        int index = 1;
            for (int i = 0; i < paths.length; i++) {
                if (paths[i].getAbsolutePath().startsWith(EnvironmentEx.getExternalStoragePath().toString())) {
                    index =i;
                    break;
                }
            }
    ...
    public boolean checkSdCardState() {
        if (IsSdCardMounted() == false) {
            showCenterToast(R.string.sd_card_not_mounted, Toast.LENGTH_SHORT);
            return false;
        }
        ...
        if (isAvaliableSpace(sdCardDirectory)) {
            ...
            showCenterToast(R.string.sd_card_lack_space, Toast.LENGTH_SHORT);
            return false;
        }
    }
    ...
    private void showCenterToast(int hint, int duration) {
        mToast = Utils.showToast(mContext, mToast,
                mContext.getString(hint), duration);
    }
